...
---
### 🔵 그리디
> 매 단계마다 현재 상황에서 가장 최선이라고 판단되는 선택을 하는 알고리즘 설계 기법이다.  
> 전체 결과를 고려해 모든 경우를 탐색하지 않고,  
> 지금 당장 이득이 되는 선택을 반복하여 해답에 도달한다.
> 
> 구현이 단순하고 실행 속도가 빠르다는 장점이 있으나,  
> 항상 최적해를 보장하지는 않기 때문에  
> 문제에서 그리디 적용 조건이 성립하는지 확인이 필요하다.
